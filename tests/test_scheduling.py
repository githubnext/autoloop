...
# ---------------------------------------------------------------------------
# findExistingPRForBranch helper — tolerant lookup of the open draft PR
# ---------------------------------------------------------------------------
...
from conftest import _JS_MODULE_PATH


def _run_find_existing_pr(program, mock_responses):
    """Invoke findExistingPRForBranch with a stubbed global fetch.

    `mock_responses` is a list of dicts: { url_match, status, body, link }.
    The first entry whose url_match substring is in the requested URL wins.
    The optional `link` field is returned as the Link response header
    (used by pagination via parseLinkHeader).
    """
    script = (
        "const m = require(" + json.dumps(_JS_MODULE_PATH) + ");\n"
        "const responses = " + json.dumps(mock_responses) + ";\n"
        "global.fetch = async (url, opts) => {\n"
        "  for (const r of responses) {\n"
        "    if (url.indexOf(r.url_match) !== -1) {\n"
        "      return {\n"
        "        ok: r.status >= 200 && r.status < 300,\n"
        "        status: r.status,\n"
        "        json: async () => r.body,\n"
        "        headers: { get: (h) => h.toLowerCase() === 'link' ? (r.link || null) : null },\n"
        "      };\n"
        "    }\n"
        "  }\n"
        ...
        "};\n"
        "(async () => {\n"
        "  const pr = await m.findExistingPRForBranch('owner/repo', " + json.dumps(program) + ", 'TOKEN');\n"
    ...
    if result.returncode != 0:
        raise RuntimeError("Node error: " + result.stderr)
