...
1. If CI is expected to run the same evaluation or publish the fitness value, commit and push the changes through Step 5a with the metric marked `pending-ci`. After CI succeeds, parse the CI-measured fitness from the run/check logs or artifacts and continue through Step 5c using that value. If the value is not available before the iteration ends, leave the iteration history metric as `pending-ci`, append `"pending-ci"` to `recent_statuses`, update the PR/issue to say the metric is awaiting CI, and reconcile it at the start of the next iteration before proposing new work.
...
| Recent Statuses | comma-separated words | Last 10 outcomes: `accepted`, `rejected`, `error`, `pending-ci`, or `ci-fix-exhausted`. The `ci-fix-exhausted` value is the coarse bucket for *any* iteration that ended because the CI gate could not be made green within the per-iteration budget — including no-progress-guard trips, 5-attempt budget exhaustion, and `ci-timeout`. The fine-grained reason is in `pause_reason`; `pending-ci` means a pushed iteration is waiting for CI-measured fitness to be reconciled. |
...
Cumulative improvement is also **direction-aware**: for `higher`, use `best_metric - initial_metric`; for `lower`, use `initial_metric - best_metric`. The percentage is `(absolute_delta / abs(initial_metric)) * 100`; report `n/a` if the initial metric is missing or zero.
